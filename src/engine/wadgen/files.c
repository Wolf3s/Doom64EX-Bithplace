...
int File_Read(char const *name, cache * buffer)
{
	...
	if (handle == -1) {
		sprintf(fullpath, "/usr/share/games/doom64ex-plus/%s", name);
		handle = open(fullpath, O_RDONLY | O_BINARY, 0666);

		if (handle == -1) {
			sprintf(fullpath, "/usr/local/share/games/doom64ex-plus/%s",
				name);
			handle = open(fullpath, O_RDONLY | O_BINARY, 0666);

			if (handle == -1) {
				sprintf(fullpath, "/usr/local/share/doom64ex-plus/%s",
					name);
				handle =
